...
ImageMark will automatically use the built-in [`shape`](/en/api/plugin/shape) , [`selection`](/en/api/plugin/selection) , [`history`](/en/api/plugin/history) , [`shortcut`](/en/api/plugin/shortcut) , [`minimap`](/en/api/plugin/minimap) plugins when importing. If you do not wish to use them automatically, you can use the [`unuseDefaultPlugin`](/en/api/constructor-methods#unusedefaultplugin) method to disable.
